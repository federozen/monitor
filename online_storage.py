"""Persistencia online del Monitor Deportivo en Google Sheets.

La configuración por defecto está pensada para una planilla nueva dedicada al
monitor: crea las hojas con sus nombres editoriales definitivos, sin prefijo.
``SHEET_PREFIX`` sigue disponible como opción avanzada, pero puede dejarse vacío.
"""
...
DESCUBRIMIENTOS_HEADERS = [
    "RunTS", "DiscoveryID", "Categoria", "Estado", "Score", "Noticiabilidad", "Confianza",
    "Senales", "MotivoConfianza", "ValorArgentina", "Titulo", "URL", "Publishers", "Medios",
    "FechaPublicacion", "TipoFecha", "AntiguedadHoras", "EsNuevo", "EstadoOle", "TituloOle",
    "URLOle", "Motivo", "PorQueImporta", "Angulo", "Formato", "EvidenciaJSON",
]
CAMBIOS_HEADERS = [
...
OLE_HOY_HEADERS = [
    "OleID", "PrimeraDeteccion", "UltimaDeteccion", "TipoRegistro", "ConfianzaFecha",
    "Origen", "Pagina", "FechaPublicacion", "FechaActualizacion", "Seccion", "TemaID",
    "TemaAgrupado", "Enfoque", "Titulo", "URL", "Entidades", "NovedadesExternas",
    "AccionSugerida"
]
...
HALLAZGOS_EDITOR_HEADERS = [
    "Corte", "EstadoHallazgo", "Prioridad", "Noticiabilidad", "Confianza", "Senales",
    "MotivoConfianza", "Tema", "QuePaso", "PorQueImporta", "EstadoOle", "Accion",
    "Formato", "Fuentes", "URLsFuentes", "URLPrincipal"
]
AUDITORIA_HEADERS = [
    "Corte", "Tipo", "ItemID", "EstadoActualidad", "Destino", "Tema", "FechaReferencia",
    "TipoFecha", "Decision", "Motivo", "Fuentes", "URL"
]
...
CONFIGURACION_HEADERS = ["Clave", "Valor", "Descripcion"]
CONFIGURACION_DEFAULTS = [
    ["zona_horaria", "America/Argentina/Buenos_Aires", "Zona horaria de todos los cortes"],
    ["frecuencia_minutos", "30", "Frecuencia objetivo de recoleccion"],
    ["max_antiguedad_horas", "4", "Ventana principal del resumen gratuito"],
    ["min_fuentes_sanas_pct", "0.60", "Umbral para considerar completo un corte"],
    ["max_temas_resumen", "40", "Cantidad maxima de temas en RESUMEN_4H"],
    ["umbral_medios", "4", "Minimo de medios originales para elevar una alerta operativa"],
    ["watchlist", "river, boca, seleccion argentina", "Temas a vigilar siempre, separados por coma"],
    ["horas_silencio", "48", "No repetir el mismo aviso heredado dentro de estas horas"],
    ["dias_archivo", "3", "Conservado por compatibilidad; no mueve las hojas nuevas"],
    ["ignorar", "", "Palabras o temas a excluir, separados por coma"],
    ["criterios_editor", "", "Criterios adicionales escritos por el editor"],
    ["digest_ole", "no", "El digest heredado permanece apagado"],
    ["avisos_explosion", "no", "Las alertas heredadas permanecen apagadas"],
    ["umbral_explosion", "4", "Aumento de medios para considerar crecimiento fuerte"],
    ["entidades_extra", "", "Entidades adicionales y alias definidos por el editor"],
    ["automatizacion", "false", "El cron se controla desde AUTOMATION_ENABLED en GitHub"],
    ["ia_automatica", "false", "La IA paga solo puede ejecutarse bajo demanda"],
]
...
def prefix() -> str:
    raw = _CONF["prefix"] if _CONF["prefix"] is not None else os.environ.get("SHEET_PREFIX", "")
...
def _apply_sheet_visibility() -> None:
    """Oculta hojas técnicas y deja visibles las salidas editoriales."""
    raw = os.environ.get("HIDE_TECHNICAL_SHEETS", "true").strip().lower()
    if raw not in {"1", "true", "yes", "si", "sí", "on"}:
        return
    technical = {
        "Noticias", "Temas", "Fuentes", "Control", "Feedback", "Recomendaciones",
        "Descubrimientos", "Cambios", "Resumen", "Oportunidades", "Informes",
        "Avisos", "AgentLog", "HISTORIAL_4H", "AUDITORIA", "CONFIGURACION",
    }
    visible = {
        "RESUMEN_4H", "ACCIONES", "OLE_HOY", "COBERTURA_OLE", "HALLAZGOS",
        "FUENTES_EDITOR", "BUZON_SOCIAL", "PARTES_IA",
    }
    requests = []
    worksheets = _worksheet_map()
    for base in technical | visible:
        ws = worksheets.get(nombre_pestana(base))
        if ws is None:
            continue
        requests.append({
            "updateSheetProperties": {
                "properties": {"sheetId": ws.id, "hidden": base in technical},
                "fields": "hidden",
            }
        })
    if requests:
...
def asegurar_estructura() -> None:
    ...
    _ws("AUDITORIA", AUDITORIA_HEADERS, rows=1500)
    config_ws = _ws("CONFIGURACION", CONFIGURACION_HEADERS, rows=80)
    try:
        values = _api_call(config_ws.get_all_values)
        if len(values) <= 1:
            _api_call(lambda: config_ws.update(
                range_name="A1",
                values=[CONFIGURACION_HEADERS] + CONFIGURACION_DEFAULTS,
                value_input_option="RAW",
            ))
            _invalidate_records("CONFIGURACION")
    except Exception:
        pass
    _apply_sheet_visibility()
...
def guardar_agente_snapshot(recommendations: list[dict], discoveries: list[dict], opportunities: list[dict]) -> dict:
    ...
    for item in discoveries:
        discovery_rows.append([
            ...
            item.get("score", 0), item.get("noticiability", item.get("score", 0)),
            item.get("confidence", 0), " | ".join(item.get("signals", []) or []),
            item.get("confidence_reason", ""), item.get("value_argentina", 0),
            item.get("title", ""), item.get("url", ""),
            " | ".join(item.get("publishers", []) or []), item.get("media_count", 0),
            item.get("published_at", ""), item.get("date_trust", ""),
            item.get("age_hours", ""), item.get("is_new", False), item.get("ole_status", ""),
            item.get("ole_match_title", ""), item.get("ole_match_url", ""), item.get("reason", ""),
            item.get("why_it_matters", ""), item.get("suggested_angle", ""),
            item.get("suggested_format", ""), item.get("evidence", []),
        ])
    opp_rows = []
...
# ---------------------------------------------------------------------------
# V12: editor-facing sheets and on-demand AI parts
# ---------------------------------------------------------------------------
...
def leer_configuracion() -> dict:
    """Lee la configuración nativa sin crear hojas heredadas."""
    rows = _records("CONFIGURACION", CONFIGURACION_HEADERS)
    return {row.get("Clave", ""): row.get("Valor", "") for row in rows if row.get("Clave")}


def leer_config_editorial() -> dict:
    """Devuelve la configuración tipada que usa el proceso automático."""
    raw = leer_configuracion()

    def as_int(key: str, default: int) -> int:
        try:
            return int(str(raw.get(key, default)).strip())
        except Exception:
            return default

    def as_bool(key: str, default: bool) -> bool:
        value = str(raw.get(key, "si" if default else "no")).strip().lower()
        return value in {"1", "true", "yes", "si", "sí", "on"}

    def as_list(key: str) -> list[str]:
        return [item.strip().lower() for item in str(raw.get(key, "")).split(",") if item.strip()]

    return {
        "umbral_medios": as_int("umbral_medios", 4),
        "watchlist": as_list("watchlist"),
        "horas_silencio": as_int("horas_silencio", 48),
        "dias_archivo": as_int("dias_archivo", 3),
        "ignorar": as_list("ignorar"),
        "criterios": str(raw.get("criterios_editor", "")),
        "digest_ole": as_bool("digest_ole", False),
        "avisos_explosion": as_bool("avisos_explosion", False),
        "umbral_explosion": as_int("umbral_explosion", 4),
        "entidades_extra": str(raw.get("entidades_extra", "")),
    }
...
def guardar_mesa_editorial(desk: dict, ole_entries: list[dict], ole_coverage: list[dict],
    ...
    Las acciones marcadas como HECHO o DESCARTADO no reaparecen si no cambió el
    hecho editorial que originó su ActionID. Las acciones abiertas se conservan
    aunque dejen de estar entre las primeras del corte actual.
    """
    asegurar_estructura()
    ...
    ole_rows = [[
        ...
        item.get("record_type", ""), item.get("date_confidence", ""), item.get("origin", ""),
        item.get("source_page", ""), item.get("published_at", ""), item.get("updated_at", ""),
        item.get("section", ""), item.get("topic_id", ""), item.get("topic", ""), item.get("focus", ""),
    ...
    coverage_rows = [[
        ...
        item.get("published_today", 0), item.get("updated_today", 0),
    ...
    hallazgo_rows = [[
        ...
        " | ".join(item.get("signals", []) or []) if isinstance(item.get("signals"), list) else item.get("signals", ""),
    ...
    audit_rows = [[
        item.get("cut_key", cut_key), item.get("item_type", ""), item.get("item_id", ""),
        item.get("freshness_state", ""), item.get("destination", ""), item.get("topic", ""),
        item.get("reference_date", ""), item.get("date_type", ""), item.get("decision", ""),
        item.get("reason", ""), item.get("sources", ""), item.get("url", ""),
    ] for item in audit]
    ...
    _format_editorial_sheet("COBERTURA_OLE", [150,320,70,85,85,180,180,145,145,320,230,480,420,180,110], {"red": 0.1, "green": 0.55, "blue": 0.22})
    _format_editorial_sheet("HALLAZGOS", [110,145,80,100,90,260,260,320,420,400,150,120,180,240,240,230], {"red": 0.55, "green": 0.2, "blue": 0.7})
