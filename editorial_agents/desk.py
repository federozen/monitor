...
def build_editorial_desk(themes: list[dict], changes: list[dict], recommendations: list[dict],
    ...
    for theme in themes or []:
        ...
        evidence = _evidence_from_theme(theme)
        trusted_dates = _activity_dates(theme, trusted_only=True)
        audit.append({
            "cut_key": cut_key, "item_type": "TEMA", "item_id": cid,
            "freshness_state": "CONFIRMADA_O_PROBABLE" if trusted_dates else "PARA_VERIFICAR",
            "destination": "ELEGIBLE_RESUMEN_4H" if eligible else "EXCLUIDO",
            "topic": _title(theme),
            "reference_date": max(trusted_dates).isoformat(timespec="minutes") if trusted_dates else "",
            "date_type": "publisher_timestamp" if trusted_dates else "sin_fecha_verificable",
            "decision": _action(rec_map.get(cid), theme),
            "reason": reason,
            "sources": _source_line(evidence)[0],
            "url": _url(theme),
        })
    ...
    for discovery in discoveries or []:
        ...
        audit.append({
            ...
            "freshness_state": "CONFIRMADA_O_PROBABLE" if fresh else "PARA_VERIFICAR",
            "destination": "RESUMEN_4H" if fresh and firm else "CANDIDATOS",
            "topic": title, "reference_date": str(discovery.get("published_at") or discovery.get("FechaPublicacion") or ""),
            "date_type": str(discovery.get("date_trust") or ""),
            "decision": ("PROFUNDIZAR" if finding_status == "HALLAZGO FUERTE" else "SEGUIR") if firm else "NO_RECOMENDACION_FIRME",
            "reason": str(discovery.get("reason") or discovery.get("Motivo") or finding_status),
            "sources": str(discovery.get("Publishers") or " | ".join(discovery.get("publishers", []) or [])),
            "url": _url(discovery),
        })
        if len(selected) >= max_topics or not fresh or not firm:
            continue
        ...
        selected.append({
            ...
            "finding_status": finding_status,
            "noticiability": _int(discovery.get("noticiability") or discovery.get("Score") or discovery.get("score") or 0),
            "confidence": _int(discovery.get("confidence") or discovery.get("Confianza") or 0),
            "signals": discovery.get("signals") or discovery.get("Senales") or [],
            "confidence_reason": str(discovery.get("confidence_reason") or discovery.get("MotivoConfianza") or ""),
            "suggested_format": str(discovery.get("suggested_format") or discovery.get("Formato") or "NOTA BREVE"),
    ...
    for item in actionable:
        actions.append({
            "action_id": stable_id(f"{item['topic_id']}|{item['action']}|{normalize_text(item['what_changed'])}", "act"),
            "cut_key": cut_key,
