...
def _action(theme: dict, agenda_item: dict, official_count: int, rumor: bool, freshness: dict) -> str:
    ...
    if coverage == "YA_CUBIERTO":
        return "OBSERVAR"
    if coverage == "CUBIERTO_CON_DATO_NUEVO":
        return "ACTUALIZAR"
    if coverage in {"CUBIERTO_PARCIALMENTE", "COINCIDENCIA_DUDOSA"}:
        return "VERIFICAR"
...
def curate(themes: list[dict], agenda: list[dict], config: dict | None = None) -> list[dict]:
    ...
    for position, theme in enumerate(themes or [], start=1):
        ...
        priority += 20 if coverage == "CUBIERTO_CON_DATO_NUEVO" else 0
        priority += 10 if coverage == "CUBIERTO_PARCIALMENTE" else 0
        ...
        elif coverage == "CUBIERTO_CON_DATO_NUEVO":
        ...
        elif coverage == "CUBIERTO_PARCIALMENTE":
            reasons.append("Olé cubrió parte del tema, pero el enfoque o el alcance no parecen equivalentes")
        elif coverage == "COINCIDENCIA_DUDOSA":
            reasons.append("hay una coincidencia dudosa con una nota de Olé y requiere revisión humana")
        else:
        ...
        recommendations.append({
            ...
            "has_ole": coverage in {"YA_CUBIERTO", "CUBIERTO_PARCIALMENTE", "CUBIERTO_CON_DATO_NUEVO"},
            "momentum": delta,
            "is_new": is_new,
