"""Piloto automático del Monitor Deportivo.

GitHub Actions recolecta fuentes, compara el corte con la memoria online,
construye la mesa editorial y escribe las hojas nuevas por lote. Las rutas
heredadas de Agenda/Snapshot solo se activan si se habilitan explícitamente.
Sin credenciales de Sheets corre en modo simulacro.
"""
import os
import sys
import time
...
def main():
    ...
    print("=== MONITOR DEPORTIVO - mesa editorial y memoria de Olé ===", "(modo simulacro: sin Sheet configurado)" if simulacro else "")
    ...
    cfg = online.leer_config_editorial() if not simulacro and online.disponible() else {
        "umbral_medios": 4, "watchlist": [], "horas_silencio": 48,
        "dias_archivo": 3, "ignorar": [], "criterios": "",
        "digest_ole": False, "avisos_explosion": False,
        "umbral_explosion": 4, "entidades_extra": "",
    }
    ...
    if not simulacro and online.disponible():
        try:
            ...
            prev = []
    if not prev and not simulacro and legacy_memory:
        try:
    ...
    if not simulacro:
        ...
        # prueba V12 no se escriben las pestanas heredadas.
        try:
    ...
    # Snapshot para la app liviana. Usa las pestañas nuevas de la planilla dedicada,
    ...
    if not simulacro and online.disponible():
        try:
            ...
            allow_degraded = os.environ.get("ALLOW_EDITORIAL_DEGRADED", "false").strip().lower() in {"1", "true", "yes", "si", "sí", "on"}
            if not allow_degraded:
                raise RuntimeError(f"Falló la persistencia central del snapshot: {exc}") from exc

    # Capa editorial: resumen 4H, memoria de Ole, acciones y hallazgos.
    ...
    if not simulacro and online.disponible():
        try:
            ...
            allow_degraded = os.environ.get("ALLOW_EDITORIAL_DEGRADED", "false").strip().lower() in {"1", "true", "yes", "si", "sí", "on"}
            if not allow_degraded:
                raise RuntimeError(f"Falló un componente central del asistente editorial: {exc}") from exc
    ...
    # La IA paga no existe en el workflow automático.
    # Solo se ejecuta desde Streamlit, con confirmación explícita.
