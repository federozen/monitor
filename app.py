"""Online editorial monitor and proactive assistant.
...
def page_now(data: dict) -> None:
    ...
    with tab_findings:
        ...
        if not discoveries:
            st.warning("No hubo material internacional útil en las fuentes del corte. Revisá FUENTES_EDITOR y los radares de descubrimiento.")
...
def page_desk(data: dict) -> None:
    ...
    ole_published_today = sum(
        ...
        if row.get("TipoRegistro") == "PUBLICADA_HOY"
        or (not row.get("TipoRegistro") and _row_date(row.get("FechaPublicacion")) == today_ar)
    )
    ole_updated_only = sum(
        1 for row in ole_today
        if row.get("TipoRegistro") == "ACTUALIZADA_HOY"
        or (not row.get("TipoRegistro") and _row_date(row.get("FechaActualizacion")) == today_ar
    ...
    tabs = st.tabs(["Resumen 4H", "Acciones", "Olé hoy", "Hallazgos", "Fuentes", "Auditoría", "Parte ampliado"])
    with tabs[0]:
    ...
    with tabs[3]:
        ...
        for row in findings[:30]:
            with st.container(border=True):
                ...
                st.caption(
                    f"{row.get('EstadoHallazgo','HALLAZGO')} · prioridad {row.get('Prioridad','')} · "
                    f"noticiabilidad {row.get('Noticiabilidad','')} · confianza {row.get('Confianza','')} · "
                    f"{row.get('Accion','')} · Olé: {row.get('EstadoOle','')}"
                )
                st.write(row.get("QuePaso", ""))
                if row.get("PorQueImporta"):
                    st.write(f"**Valor para Argentina:** {row.get('PorQueImporta')}")
                if row.get("Senales"):
                    st.write(f"**Señales editoriales:** {row.get('Senales')}")
                if row.get("MotivoConfianza"):
                    st.write(f"**Por qué se confía:** {row.get('MotivoConfianza')}")
                if row.get("Formato"):
                    st.write(f"**Formato sugerido:** {row.get('Formato')}")
                st.caption(f"Fuentes: {row.get('Fuentes','')}")
    with tabs[4]:
    ...
    with tabs[5]:
        ...
        a1.metric("Incluidos", sum(1 for row in audit if row.get("Destino") == "ELEGIBLE_RESUMEN_4H" or row.get("Destino") == "RESUMEN_4H"))
        a2.metric("Para verificar", sum(1 for row in audit if row.get("EstadoActualidad") == "PARA_VERIFICAR"))
        a3.metric("Excluidos", sum(1 for row in audit if row.get("Destino") == "EXCLUIDO"))
        st.caption("Explica qué fecha se usó, por qué entró o quedó afuera y qué decisión tomó el sistema.")
        st.dataframe(audit[:500], use_container_width=True, hide_index=True)
    with tabs[6]:
...
def page_config(data: dict) -> None:
    ...
    config = store.leer_configuracion()
    if config:
...
with st.sidebar:
    st.header("Monitor Deportivo")
    page = st.radio(
        "Ir a",
        ["Mesa editorial", "Buzón social", "Ahora técnico", "Asistente", "Explorar", "Producir", "Configuracion"],
    )
    st.divider()
